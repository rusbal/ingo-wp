...
add_action( 'widgets_init', 'wpsight_stage_register_widget_listing_title' );
 
function wpsight_stage_register_widget_listing_title() {
	register_widget( 'WPSight_STAGE_Listing_Title' );
}
...
class WPSight_STAGE_Listing_Title extends WP_Widget {

	public function __construct() {

		$widget_ops = array(
			'classname'   => 'widget_listing_title',
			'description' => _x( 'Display title and listing actions on single listing pages.', 'listing wigdet', 'wpsight-stage' )
		);

		parent::__construct( 'wpsight_stage_listing_title', '&rsaquo; ' . WPSIGHT_STAGE_NAME . ' ' . _x( 'Listing Title', 'listing widget', 'wpsight-stage' ), $widget_ops );
	...
	public function form( $instance ) {
		...
		if( is_array( $actions ) ) { ?>
			<p><?php _e( 'Please select the listing actions that you want to be displayed along the title.', 'wpsight-stage' ); ?></p><?php			
		} else { ?>		
			<p><?php _e( 'Currently there are no listing actions to be displayed along the title.', 'wpsight-stage' ); ?></p><?php		
		}
