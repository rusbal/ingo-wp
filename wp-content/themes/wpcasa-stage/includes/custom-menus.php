...
function wpsight_stage_menus() {

	$menus = array(
		'primary' => __( 'Primary Menu', 'wpsight-stage' ),
		'secondary' => __( 'Secondary Menu', 'wpsight-stage' )
	);
	
	return apply_filters( 'wpsight_stage_menus', $menus );

}
 
/**
 * wpsight_stage_register_menus()
 *
 * Register custom menus by looping
 * through theme menus.
 *
 * @uses wpsight_stage_menus()
 * @uses register_nav_menu()
 *
 * @since 1.0.0
 */
 
add_action( 'after_setup_theme', 'wpsight_stage_register_menus' );

function wpsight_stage_register_menus() {

	foreach( wpsight_stage_menus() as $menu => $label )	
...
if ( ! function_exists( 'wpsight_stage_menu' ) ) {

	function wpsight_stage_menu( $menu_location = 'primary', $menu_args = array(), $menu_default = false ) {
		...
		$menu_args = apply_filters( 'wpsight_stage_menu_args', wp_parse_args( $menu_args, $menu_defaults ), $menu_location, $menu_args, $menu_default );
		...
		if( ! has_nav_menu( $menu_location ) && $menu_default === true ) {
			...
			$menu_fallback = '<ul class="wpsight-menu">
				<li class="menu-item">
					<a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">' . __( 'Create a custom menu &rarr;', 'wpsight-stage' ) . '</a>
				</li>
			</ul>';
...
add_filter( 'wp_nav_menu', 'wpsight_stage_menu_first_last' );

function wpsight_stage_menu_first_last( $menu ) {
