<?php
/**
 * WPCasa Elviria listings carousel
 *
 * @package WPCasa Elviria
 */

/**
 * wpsight_elviria_listings_carousel()
 *
 * Echo wpsight_elviria_get_listings_carousel()
 *
 * @param object|array $listings Query object or post IDs array
 * @param array $args Optional carousel arguments
 *
 * @uses wpsight_elviria_get_listings_carousel()
 *
 * @since 1.0.0
 */
 
function wpsight_elviria_listings_carousel( $listings = array(), $args = array() ) {
	echo wpsight_elviria_get_listings_carousel( $listings, $args );
}

/**
 * wpsight_elviria_get_listings_carousel()
 *
...
function wpsight_elviria_get_listings_carousel( $listings = array(), $args = array() ) {
	global $post;
	...
	if( $listings ) {
		...
		$args = apply_filters( 'wpsight_elviria_get_listings_carousel_args', wp_parse_args( $args, $defaults ), $listings, $args );
		...
		<div class="<?php echo $class_carousel; ?>">
			...
			<?php foreach( $listings as $listing ) : $post = get_post( $listing ); setup_postdata( $post ); ?>
				...
				<?php wpsight_get_template( 'listing-carousel.php', array( 'post' => $post, 'counter' => $i ) ); ?>
		...
		return apply_filters( 'wpsight_elviria_get_listings_carousel', ob_get_clean(), $listings, $args );
