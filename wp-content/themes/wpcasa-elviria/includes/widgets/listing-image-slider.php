...
add_action( 'widgets_init', 'wpsight_elviria_register_widget_listing_image_slider' );
 
function wpsight_elviria_register_widget_listing_image_slider() {
	register_widget( 'WPSight_Elviria_Listing_Image_Slider' );
}
...
class WPSight_Elviria_Listing_Image_Slider extends WP_Widget {

	public function __construct() {

		$widget_ops = array(
			'classname'   => 'widget_listing_image_slider',
			'description' => _x( 'Display listing images in thumbnail slider.', 'listing wigdet', 'wpcasa-elviria' )
		);

		parent::__construct( 'wpsight_elviria_listing_image_slider', '&rsaquo; ' . WPSIGHT_ELVIRIA_NAME . ' ' . _x( 'Listing Image Slider', 'listing widget', 'wpcasa-elviria' ), $widget_ops );
	...
	public function widget( $args, $instance ) {
		...
		wpsight_elviria_image_slider( get_the_id(), $slider_args );
	...
	public function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title', 'wpcasa-elviria' ); ?>: <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></label></p>
		
		<p><label for="<?php echo $this->get_field_id( 'slider_items' ); ?>"><?php _e( 'Items', 'wpcasa-elviria' ); ?>:</label><br />
			<select class="widefat" id="<?php echo $this->get_field_id( 'slider_items' ); ?>" name="<?php echo $this->get_field_name( 'slider_items' ); ?>">
				<option value="1"<?php selected( $slider_items, 1 ); ?>>1 <?php _ex( 'item', 'listing widget', 'wpcasa-elviria' ); ?></option>
				<option value="2"<?php selected( $slider_items, 2 ); ?>>2 <?php _ex( 'items', 'listing widget', 'wpcasa-elviria' ); ?></option>
				<option value="3"<?php selected( $slider_items, 3 ); ?>>3 <?php _ex( 'items', 'listing widget', 'wpcasa-elviria' ); ?></option>
				<option value="4"<?php selected( $slider_items, 4 ); ?>>4 <?php _ex( 'items', 'listing widget', 'wpcasa-elviria' ); ?></option>
				<option value="5"<?php selected( $slider_items, 5 ); ?>>5 <?php _ex( 'items', 'listing widget', 'wpcasa-elviria' ); ?></option>
				<option value="6"<?php selected( $slider_items, 6 ); ?>>6 <?php _ex( 'items', 'listing widget', 'wpcasa-elviria' ); ?></option>
			</select><br />
			<span class="description"><?php _e( 'Please select the number of visible slider items', 'wpcasa-elviria' ); ?></span></p>
		
		<div<?php if( $slider_items == 1 ) echo ' style="display:none"'; ?>>
			
			<p><label for="<?php echo $this->get_field_id( 'slider_slide_by' ); ?>"><?php _e( 'Slide by', 'wpcasa-elviria' ); ?>:</label><br />
				<select class="widefat" id="<?php echo $this->get_field_id( 'slider_slide_by' ); ?>" name="<?php echo $this->get_field_name( 'slider_slide_by' ); ?>">
					<option value="1"<?php selected( $slider_slide_by, 1 ); ?>>1 <?php _ex( 'item', 'listing widget', 'wpcasa-elviria' ); ?></option>
					<option value="2"<?php selected( $slider_slide_by, 2 ); if( $slider_items < 2 ) echo ' style="display:none"'; ?>>2 <?php _ex( 'items', 'listing widget', 'wpcasa-elviria' ); ?></option>
					<option value="3"<?php selected( $slider_slide_by, 3 ); if( $slider_items < 3 ) echo ' style="display:none"'; ?>>3 <?php _ex( 'items', 'listing widget', 'wpcasa-elviria' ); ?></option>
					<option value="4"<?php selected( $slider_slide_by, 4 ); if( $slider_items < 4 ) echo ' style="display:none"'; ?>>4 <?php _ex( 'items', 'listing widget', 'wpcasa-elviria' ); ?></option>
					<option value="5"<?php selected( $slider_slide_by, 5 ); if( $slider_items < 5 ) echo ' style="display:none"'; ?>>5 <?php _ex( 'items', 'listing widget', 'wpcasa-elviria' ); ?></option>
					<option value="6"<?php selected( $slider_slide_by, 6 ); if( $slider_items < 6 ) echo ' style="display:none"'; ?>>6 <?php _ex( 'items', 'listing widget', 'wpcasa-elviria' ); ?></option>
				</select><br />
				<span class="description"><?php _e( 'Please select the number items to slide', 'wpcasa-elviria' ); ?></span></p>
		
		</div>
		
		<p><label for="<?php echo $this->get_field_id( 'slider_margin' ); ?>"><?php _e( 'Margin', 'wpcasa-elviria' ); ?>:</label><br />
			...
			<span class="description"><?php _e( 'Please select the space between slider items', 'wpcasa-elviria' ); ?></span></p>
		
		<p><label for="<?php echo $this->get_field_id( 'slider_stage_padding' ); ?>"><?php _e( 'Stage padding', 'wpcasa-elviria' ); ?>:</label><br />
			...
			<span class="description"><?php _e( 'Please select the stage padding (left and right space to see neighbour items)', 'wpcasa-elviria' ); ?></span></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'slider_loop' ); ?>" name="<?php echo $this->get_field_name( 'slider_loop' ); ?>"<?php checked( $slider_loop ); ?> />
				<label for="<?php echo $this->get_field_id( 'slider_loop' ); ?>"><?php _e( 'Loop slider to be infinite', 'wpcasa-elviria' ); ?></label></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'slider_nav' ); ?>" name="<?php echo $this->get_field_name( 'slider_nav' ); ?>"<?php checked( $slider_nav ); ?> />
				<label for="<?php echo $this->get_field_id( 'slider_nav' ); ?>"><?php _e( 'Show prev/next slider navigation', 'wpcasa-elviria' ); ?></label></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'slider_dots' ); ?>" name="<?php echo $this->get_field_name( 'slider_dots' ); ?>"<?php checked( $slider_dots ); ?> />
				<label for="<?php echo $this->get_field_id( 'slider_dots' ); ?>"><?php _e( 'Show <em>dots</em> slider navigation', 'wpcasa-elviria' ); ?></label></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'slider_autoplay' ); ?>" name="<?php echo $this->get_field_name( 'slider_autoplay' ); ?>"<?php checked( $slider_autoplay ); ?> />
				<label for="<?php echo $this->get_field_id( 'slider_autoplay' ); ?>"><?php _e( 'Slide through items automatically', 'wpcasa-elviria' ); ?></label></p>
		
		<div<?php if( ! $slider_autoplay ) echo ' style="display:none"'; ?>>
		
			<p><label for="<?php echo $this->get_field_id( 'slider_autoplay_time' ); ?>"><?php _e( 'Autoplay interval', 'wpcasa-elviria' ); ?>:</label><br />
				<select class="widefat" id="<?php echo $this->get_field_id( 'slider_autoplay_time' ); ?>" name="<?php echo $this->get_field_name( 'slider_autoplay_time' ); ?>">
					<option value="2000"<?php selected( $slider_autoplay_time, 2000 ); ?>>2 <?php _ex( 'seconds', 'listing widget', 'wpcasa-elviria' ); ?></option>
					<option value="4000"<?php selected( $slider_autoplay_time, 4000 ); ?>>4 <?php _ex( 'seconds', 'listing widget', 'wpcasa-elviria' ); ?></option>
					<option value="6000"<?php selected( $slider_autoplay_time, 6000 ); ?>>6 <?php _ex( 'seconds', 'listing widget', 'wpcasa-elviria' ); ?></option>
					<option value="8000"<?php selected( $slider_autoplay_time, 8000 ); ?>>8 <?php _ex( 'seconds', 'listing widget', 'wpcasa-elviria' ); ?></option>
					<option value="10000"<?php selected( $slider_autoplay_time, 10000 ); ?>>10 <?php _ex( 'seconds', 'listing widget', 'wpcasa-elviria' ); ?></option>
				</select><br />
				<span class="description"><?php _e( 'Please select the autoplay interval timeout', 'wpcasa-elviria' ); ?></span></p>
		
		</div>
		
		<p><label for="<?php echo $this->get_field_id( 'thumbs_size' ); ?>"><?php _e( 'Thumbnail Size', 'wpcasa-elviria' ); ?>:</label><br />
			...
			<span class="description"><?php _e( 'Please select the thumbnail size', 'wpcasa-elviria' ); ?></span></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'thumbs_caption' ); ?>" name="<?php echo $this->get_field_name( 'thumbs_caption' ); ?>"<?php checked( $thumbs_caption ); ?> />
			<label for="<?php echo $this->get_field_id( 'thumbs_caption' ); ?>"><?php _e( 'Display thumbnail captions (if any)', 'wpcasa-elviria' ); ?></label></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'thumbs_link' ); ?>" name="<?php echo $this->get_field_name( 'thumbs_link' ); ?>"<?php checked( $thumbs_link ); ?> />
			<label for="<?php echo $this->get_field_id( 'thumbs_link' ); ?>"><?php _e( 'Link thumbnails to image file (in lightbox if active)', 'wpcasa-elviria' ); ?></label></p>
		
		<div<?php if( ! $thumbs_link || apply_filters( 'wpsight_elviria_photoswipe', true ) != true ) echo ' style="display:none"'; ?>>
		
			<p><label for="<?php echo $this->get_field_id( 'lightbox_size' ); ?>"><?php _e( 'Lightbox Size', 'wpcasa-elviria' ); ?>:</label><br />
				...
				<span class="description"><?php _e( 'Please select the image size for the lightbox', 'wpcasa-elviria' ); ?></span></p>
				...
				<label for="<?php echo $this->get_field_id( 'lightbox_caption' ); ?>"><?php _e( 'Show image captions in lightbox', 'wpcasa-elviria' ); ?></label></p>
				...
				<label for="<?php echo $this->get_field_id( 'lightbox_prev_next' ); ?>"><?php _e( 'Show prev/next navigation in lightbox', 'wpcasa-elviria' ); ?></label></p>
				...
				<label for="<?php echo $this->get_field_id( 'lightbox_zoom' ); ?>"><?php _e( 'Allow zoom option in lightbox', 'wpcasa-elviria' ); ?></label></p>
				...
				<label for="<?php echo $this->get_field_id( 'lightbox_fullscreen' ); ?>"><?php _e( 'Allow fullscreen toggle in lightbox', 'wpcasa-elviria' ); ?></label></p>
				...
				<label for="<?php echo $this->get_field_id( 'lightbox_share' ); ?>"><?php _e( 'Display sharing options in lightbox', 'wpcasa-elviria' ); ?></label></p>
				...
				<label for="<?php echo $this->get_field_id( 'lightbox_close' ); ?>"><?php _e( 'Show close button in lightbox', 'wpcasa-elviria' ); ?></label></p>
				...
				<label for="<?php echo $this->get_field_id( 'lightbox_counter' ); ?>"><?php _e( 'Show image counter in lightbox', 'wpcasa-elviria' ); ?></label></p>
