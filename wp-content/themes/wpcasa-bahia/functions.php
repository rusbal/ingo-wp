<?php
/**
 * WPCasa Bahia theme functions and definitions
 *
 ...
 * @since 1.0
 */
add_action( 'after_setup_theme', 'wpsight_bahia_constants' );

function wpsight_bahia_constants() {

	// General theme constants
	
	define( 'WPSIGHT_BAHIA_NAME', 'WPCasa Bahia' );
	define( 'WPSIGHT_BAHIA_DOMAIN', 'wpcasa-bahia' );
	define( 'WPSIGHT_BAHIA_VERSION', '1.0.2' );
 ...
 * @since 1.0
 */ 
add_action( 'after_setup_theme', 'wpsight_bahia_load_textdomain' );

function wpsight_bahia_load_textdomain() {
	load_theme_textdomain( 'wpcasa-bahia', get_template_directory() . '/languages' );
}
 ...
 */
add_action( 'wp_enqueue_scripts', 'wpsight_bahia_scripts' );

function wpsight_bahia_scripts() {
	...
	// Enqueue theme CSS
	wp_enqueue_style( 'wpsight-bahia', get_stylesheet_uri() );
	
	// Enqueue responsive menu JS
	wp_enqueue_script( 'responsive-menu', get_template_directory_uri() . '/assets/js/responsive-menu' . $suffix . '.js', array(), WPSIGHT_BAHIA_VERSION, false );
	...
	if( apply_filters( 'wpsight_bahia_owlcarousel', true ) == true ) {
	...
	if( apply_filters( 'wpsight_bahia_tiptip', true ) == true ) {		
		wp_enqueue_script( 'tiptip', get_template_directory_uri() . '/assets/js/tiptip' . $suffix . '.js', array( 'jquery', 'jquery-tiptip' ), WPSIGHT_BAHIA_VERSION, false );		
	}
	
	// Enqueue Match Height if desired
	
	if( apply_filters( 'wpsight_bahia_equal_height', true ) == true ) {
		wp_enqueue_script( 'jquery-match-height', get_template_directory_uri() . '/assets/js/jquery.matchHeight' . $suffix . '.js', array( 'jquery' ), WPSIGHT_BAHIA_VERSION, false );		
		wp_enqueue_script( 'match-height', get_template_directory_uri() . '/assets/js/match-height' . $suffix . '.js', array( 'jquery', 'jquery-match-height' ), WPSIGHT_BAHIA_VERSION, false );		
	}
	
	// Enqueue Google Fonts if desired
	
	if( apply_filters( 'wpsight_bahia_google_fonts', true ) == true ) {
 ...
 */
add_filter( 'wpsight_get_search_fields', 'wpsight_bahia_get_search_field_widths' );

function wpsight_bahia_get_search_field_widths( $fields ) {
 ...
 */
add_filter( 'wpsight_get_advanced_search_fields', 'wpsight_bahia_get_advanced_search_fields_widths' );

function wpsight_bahia_get_advanced_search_fields_widths( $fields ) {
 ...
 */
add_action( 'after_setup_theme', 'wpsight_bahia_theme_updater' );

function wpsight_bahia_theme_updater() {
