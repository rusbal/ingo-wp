...
function wpsight_bahia_menus() {

	$menus = array(
		'top'		=> __( 'Top Menu', 'wpcasa-bahia' ),
		'primary'	=> __( 'Primary Menu', 'wpcasa-bahia' )
	);
	
	return apply_filters( 'wpsight_bahia_menus', $menus );

}
 
/**
 * wpsight_bahia_register_menus()
 *
 * Register custom menus by looping
 * through theme menus.
 *
 * @uses wpsight_bahia_menus()
 * @uses register_nav_menu()
 *
 * @since 1.0.0
 */
 
add_action( 'after_setup_theme', 'wpsight_bahia_register_menus' );

function wpsight_bahia_register_menus() {

	foreach( wpsight_bahia_menus() as $menu => $label )	
...
if ( ! function_exists( 'wpsight_bahia_menu' ) ) {

	function wpsight_bahia_menu( $menu_location = 'primary', $menu_args = array(), $menu_default = false ) {
		...
		$menu_args = apply_filters( 'wpsight_bahia_menu_args', wp_parse_args( $menu_args, $menu_defaults ), $menu_location, $menu_args, $menu_default );
		...
		if( ! has_nav_menu( $menu_location ) && $menu_default === true ) {
			...
			$menu_fallback = '<ul class="wpsight-menu">
				<li class="menu-item">
					<a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">' . __( 'Create a custom menu &rarr;', 'wpcasa-bahia' ) . '</a>
				</li>
			</ul>';
...
add_filter( 'wp_nav_menu', 'wpsight_bahia_menu_first_last' );

function wpsight_bahia_menu_first_last( $menu ) {
