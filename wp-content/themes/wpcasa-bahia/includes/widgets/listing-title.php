...
add_action( 'widgets_init', 'wpsight_bahia_register_widget_listing_title' );
 
function wpsight_bahia_register_widget_listing_title() {
	register_widget( 'WPSight_Bahia_Listing_Title' );
}
...
class WPSight_Bahia_Listing_Title extends WP_Widget {

	public function __construct() {

		$widget_ops = array(
			'classname'   => 'widget_listing_title',
			'description' => _x( 'Display title and listing actions on single listing pages.', 'listing wigdet', 'wpcasa-bahia' )
		);

		parent::__construct( 'wpsight_bahia_listing_title', '&rsaquo; ' . WPSIGHT_BAHIA_NAME . ' ' . _x( 'Listing Title', 'listing widget', 'wpcasa-bahia' ), $widget_ops );
	...
	public function form( $instance ) {
		...
		if( is_array( $actions ) ) { ?>
			<p><?php _e( 'Please select the listing actions that you want to be displayed along the title.', 'wpcasa-bahia' ); ?></p><?php			
		} else { ?>		
			<p><?php _e( 'Currently there are no listing actions to be displayed along the title.', 'wpcasa-bahia' ); ?></p><?php		
		}
