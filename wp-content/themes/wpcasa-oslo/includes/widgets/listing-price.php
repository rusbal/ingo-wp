 ...
 * Register widget
 */
add_action( 'widgets_init', 'wpsight_oslo_register_widget_listing_price' );
 
function wpsight_oslo_register_widget_listing_price() {
	register_widget( 'WPSight_Oslo_Listing_Price' );
}

/**
 * Listing price widget class
 */
class WPSight_Oslo_Listing_Price extends WP_Widget {

	public function __construct() {

		$widget_ops = array(
			'classname'   => 'widget_listing_price',
			'description' => _x( 'Display info bar with price, ID and offer badge (sale, rent etc.) on single listing pages.', 'listing wigdet', 'wpcasa-oslo' )
		);

		parent::__construct( 'wpsight_oslo_listing_price', '&rsaquo; ' . _x( 'Single Listing Price', 'listing widget', 'wpcasa-oslo' ), $widget_ops );
	...
	public function form( $instance ) {
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'display_id' ); ?>" name="<?php echo $this->get_field_name( 'display_id' ); ?>"<?php checked( $display_id ); ?> />
			<label for="<?php echo $this->get_field_id( 'display_id' ); ?>"><?php _e( 'Display listing ID', 'wpcasa-oslo' ); ?></label></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'display_offer' ); ?>" name="<?php echo $this->get_field_name( 'display_offer' ); ?>"<?php checked( $display_offer ); ?> />
			<label for="<?php echo $this->get_field_id( 'display_offer' ); ?>"><?php _e( 'Display listing offer (sale/rent etc.)', 'wpcasa-oslo' ); ?></label></p><?php
