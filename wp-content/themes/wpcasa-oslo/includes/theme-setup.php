 ...
 *	@since	1.0.0
 */
add_action( 'after_setup_theme', 'wpsight_oslo_setup' );

if ( ! function_exists( 'wpsight_oslo_setup' ) ) {

	function wpsight_oslo_setup() {
	
		// Make theme available for translation
		load_theme_textdomain( 'wpcasa-oslo', get_template_directory() . '/languages' );
		...
		$background_args = array(
			'default-color' => 'f0f0f3',
			'default-image' => false
		);
		
		$background_args = apply_filters( 'wpsight_oslo_custom_background_args', $background_args );
		...
		$header_args = array(			
			...
			'header-text'            => true,
			'uploads'                => true,
			'wp-head-callback'       => 'wpsight_oslo_header_style',
			'admin-head-callback'    => '',
			'admin-preview-callback' => '',
		);
		
		$header_args = apply_filters( 'wpsight_oslo_custom_header_args', $header_args );
		...
		// Add theme print CSS
		add_action( 'wpsight_head_print', 'wpsight_oslo_head_print_css' );
	
	} // end wpsight_oslo_setup()

} // end ! function_exists()

/**
 *	wpsight_oslo_header_style()
 *	
 ...
 *	@since	1.0.0
 */
function wpsight_oslo_header_style() {
 ...
 *	@since	1.0.0
 */
add_action( 'after_setup_theme', 'wpsight_oslo_content_width', 0 );

function wpsight_oslo_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'wpsight_oslo_content_width', 1140 );
}
 ...
 *	@since	1.0.0
 */
add_filter( 'wpsight_get_search_fields', 'wpsight_oslo_get_search_field_widths' );

function wpsight_oslo_get_search_field_widths( $fields ) {
 ...
 *	@since	1.0.0
 */
add_filter( 'wpsight_get_advanced_search_fields', 'wpsight_oslo_get_advanced_search_fields_widths' );

function wpsight_oslo_get_advanced_search_fields_widths( $fields ) {
 ...
 *	@since	1.0.0
 */
add_filter( 'get_search_form', 'wpsight_oslo_search_form' );

function wpsight_oslo_search_form( $form ) {
	...
	<form role="search" method="get" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
		<div class="row gutter-10">
			<div class="col-xs-8">
				<label class="sr-only"><span class="screen-reader-text"><?php _ex( 'Search for', 'search form', 'wpcasa-oslo' ); ?></span></label>
				<input type="search" class="search-field form-control" placeholder="<?php echo esc_attr_x( 'Search&hellip;', 'search form', 'wpcasa-oslo' ); ?>" value="<?php echo esc_attr( get_search_query() ); ?>" name="s">
			</div>
			<div class="col-xs-4">
				<input type="submit" class="search-submit btn btn-primary btn-block" value="<?php echo esc_attr_x( 'Search', 'search form', 'wpcasa-oslo' ); ?>">
			</div>
		</div>
	</form>
	<?php
 ...
 */
add_filter( 'embed_oembed_html', 'wpsight_oslo_oembed_handler', 10, 4 );

function wpsight_oslo_oembed_handler( $html, $url, $attr, $post_ID ) {
 ...
 *	@since	1.0.0
 */
add_filter( 'wpsight_get_pagination_args', 'wpsight_oslo_pagination_args' );

function wpsight_oslo_pagination_args( $args ) {
 ...
 *	@since	1.0.0
 */
add_filter( 'wpsight_get_pagination', 'wpsight_oslo_pagination' );

function wpsight_oslo_pagination( $paginate_links ) {
 ...
 *	@since 1.0
 */
if ( ! function_exists( 'wpsight_oslo_comment' ) ) {

	function wpsight_oslo_comment( $comment, $args, $depth ) {
		...
		<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<div class="comment-body">
				<?php _e( 'Pingback:', 'wpcasa-oslo' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'wpcasa-oslo' ), '<span class="edit-link">', '</span>' ); ?>
			</div>
			...
			<div id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				<header class="comment-meta">
					<div class="comment-author vcard">
						...
						<?php printf( __( '%s <span class="says">says:</span>', 'wpcasa-oslo' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
					</div><!-- .comment-author -->
	
					<div class="comment-metadata">
						<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
							<time datetime="<?php comment_time( 'c' ); ?>">
								<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'wpcasa-oslo' ), get_comment_date(), get_comment_time() ); ?>
							</time>
						</a>
						<?php edit_comment_link( __( 'Edit', 'wpcasa-oslo' ), '<span class="edit-link">', '</span>' ); ?>
					</div><!-- .comment-metadata -->
	
					<?php if ( '0' == $comment->comment_approved ) : ?>
					<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'wpcasa-oslo' ); ?></p>
					<?php endif; ?>
				</header><!-- .comment-meta -->
	...
	add_filter( 'comment_reply_link', 'wpsight_oslo_comment_reply_link' );
	
	function wpsight_oslo_comment_reply_link( $link ) {		
		return str_replace( 'comment-reply-link' , 'comment-reply-link btn btn-sm btn-primary', $link );		
	}

} // endif check for wpsight_oslo_comment()
 ...
 *	@since 1.0.0
 */
add_filter( 'body_class', 'wpsight_oslo_body_class' );

function wpsight_oslo_body_class( $classes ) {
	
	$classes[] = 'wpsight-oslo';
 ...
 *	@since 1.0.0
 */
add_filter( 'gform_field_content', 'wpsight_oslo_gform_field_content', 10, 2 );

function wpsight_oslo_gform_field_content( $content, $field ) {
 ...
 *	@since 1.0.0
 */
add_filter( 'gform_submit_button', 'wpsight_oslo_gform_submit_button', 10, 2 );

function wpsight_oslo_gform_submit_button( $button, $form ) {	
	return '<input type="submit" class="btn btn-lg btn-primary" id="gform_submit_button_' . $form['id'] . ' value="' . $form['button']['text'] . '" />';
}

/**
 *	wpsight_oslo_head_print_css()
 *	
 ...
 *	@since 1.0.0
 */
function wpsight_oslo_head_print_css() {
...
add_filter( 'pre_get_posts','wpsight_oslo_search_filter' );

/**
 *	wpsight_oslo_search_filter()
 *	
 ...
 *	@since 1.0.0
 */
function wpsight_oslo_search_filter( $query ) {
...
/**
 *	wpsight_oslo_demo_import_remove_content()
 *
 ...
 *	@since	1.0.0
 */
add_action( 'wpsight_demo_import_process_content_before', 'wpsight_oslo_demo_import_remove_content' );

function wpsight_oslo_demo_import_remove_content() {
...
/**
 *	wpsight_oslo_demo_import_remove_widgets()
 *
 *	When demo content is imported we remove the
 ...
 *	@since	1.0.0
 */
add_action( 'wpsight_demo_import_process_widgets_before', 'wpsight_oslo_demo_import_remove_widgets' );

function wpsight_oslo_demo_import_remove_widgets() {
			
	if ( ! get_option( 'wpsight_oslo_cleared_widgets' ) ) {

		update_option( 'sidebars_widgets', array() );
		
		// Set flag to only run this once
		update_option( 'wpsight_oslo_cleared_widgets', true );
...
/**
 *	wpsight_oslo_demo_import_listings_map_options()
 *
 ...
 *	@since	1.0.0
 */
add_action( 'wpsight_demo_import_process_custom_options_after', 'wpsight_oslo_demo_import_listings_map_options' );

function wpsight_oslo_demo_import_listings_map_options() {
...
/**
 *	wpsight_oslo_get_call_to_action_url()
 *
 *	Replace demo content contact us URL
 *
 *	@since 1.0.0
 */
add_filter( 'wpsight_oslo_get_call_to_action_args', 'wpsight_oslo_get_call_to_action_url' );

function wpsight_oslo_get_call_to_action_url( $args ) {
	
	if( 'http://import.wpcasa.com/oslo/contact-us/' == $args['link_url'] )
...
/**
 *	wpsight_oslo_get_feature_box_url()
 *
 *	Replace demo content contact us URL
 *
 *	@since 1.0.0
 */
add_filter( 'wpsight_oslo_get_feature_box_args', 'wpsight_oslo_get_feature_box_url' );

function wpsight_oslo_get_feature_box_url( $args ) {
	
	$args['link_url'] = str_replace( 'http://import.wpcasa.com/oslo/', home_url( 'services/' ), $args['link_url'] );
...
/**
 *	wpsight_oslo_listing_agent_image()
 *
 *	Correctly display default demo agent image
 *
 *	@since 1.0.0
 */
add_filter( 'wpsight_listing_agent_image', 'wpsight_oslo_listing_agent_image', 10, 3 );

function wpsight_oslo_listing_agent_image( $image, $post, $size ) {
...
/**
 *	wpsight_oslo_get_listing_summary()
 *
 *	Add CSS class with details count to summary
 *
 *	@since 1.0.0
 */
add_filter( 'wpsight_get_listing_summary', 'wpsight_oslo_get_listing_summary', 10, 4 );

function wpsight_oslo_get_listing_summary( $listing_summary, $post_id, $details, $formatted ) {
