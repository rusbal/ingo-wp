...
if ( ! class_exists( 'WPSight_Demo_Import' ) ) {
	...
	final class WPSight_Demo_Import {
		...
		public static function admin_menu() {
			add_submenu_page( 'themes.php', sprintf( __( '%s Demo Import', 'wpcasa-oslo'), WPSIGHT_NAME ), __( 'Demo Import', 'wpcasa-oslo' ), 'manage_options', 'wpsight-demo-import', array( __CLASS__, 'template' ) );
		}
		...
		public static function admin_enqueue_scripts() {
			...
			wp_enqueue_style( 'demo-import', get_template_directory_uri() . '/assets/css/wpsight-demo-import' . $suffix . '.css', false, WPSIGHT_OSLO_VERSION, 'all'  );
			wp_enqueue_script( 'demo-import', get_template_directory_uri() . '/assets/js/wpsight-demo-import' . $suffix . '.js', false, WPSIGHT_OSLO_VERSION, true );
		...
		public static function get_import_steps() {
			...
			$import_steps = array(
				...
				'content' => array(
					...
					'title'         => __( 'WordPress Content', 'wpcasa-oslo' ),
					'description'   => __( '<strong>Please be patient!</strong> The attached images of the example listings, posts and pages are downloaded from our demo server.', 'wpcasa-oslo' ),
				...
				'theme-options' => array(
					...
					'title'         => __( 'Theme Options', 'wpcasa-oslo'),
					'description'   => __( 'Here we import options and theme modificactions from the theme customizer. Change them to your needs later.', 'wpcasa-oslo' ) . '<br />',
				...
				'widgets-content' => array(
					...
					'title'         => __( 'Widgets &amp; Settings', 'wpcasa-oslo'),
					'description'   => __( 'In this step widgets including all their settings are imported to the widget areas of this theme.', 'wpcasa-oslo' ),
				...
				'widget-logic' => array(
					...
					'title'         => __( 'Widget Logic', 'wpcasa-oslo'),
					'description'   => __( 'Next we import conditional tags of the Widget Logic plugin for more flexibility.', 'wpcasa-oslo' ) . '<br />' . sprintf( __( 'Required plugin: %s', 'wpcasa-oslo' ), '<a href="https://wordpress.org/plugins/widget-logic/" target="_blank">Widget Logic</a>' ),
				...
				*/
				...
				'custom-options' => array(
					...
					'title'         => __( 'General Options', 'wpcasa-oslo'),
					'description'   => __( 'Finally we import some custom options with basic WordPress settings.', 'wpcasa-oslo' ),
		...
		public static function template() {
		   ...
		   ?><div class="wrap">
				<h2><?php printf( __( '%s Demo Import', 'wpcasa-oslo'), WPSIGHT_NAME ); ?></h2>
				...
				<div class="wpsight-demo-import-wrapper">
					...
					<?php if ( false == self::exports_exists() ) : ?>
						...
						<div class="wpsight-demo-import-notice notice notice-error">
							...
							<p><strong><?php _e( 'Import Files Missing!', 'wpcasa-oslo' ); ?></strong>
							<br /><?php _e( 'Seems that the import files that are necessary for the import process are missing. Please check that or contact support.', 'wpcasa-oslo' ); ?></p>
					...
					<?php elseif ( true === self::is_import_complete() ) : ?>
						...
						<div class="wpsight-demo-import-notice notice notice-success">
							...
							<p><strong><?php _e( 'The import is complete!', 'wpcasa-oslo' ); ?></strong> <?php _e( 'All import steps have been processed sucessfully. Enjoy the theme!', 'wpcasa-oslo' ); ?></p>
					...
					<?php elseif ( 0 === count( self::get_missing_plugins() ) ) : ?>
						...
						<div class="wpsight-demo-import-notice notice notice-info is-dismissible">					
							<p><?php _e( 'Please do not reload the browser window during the import process.', 'wpcasa-oslo' ); ?></p>						
						</div>
					
						<a class="wpcasa-demo-import-run button button-primary button-hero">
							<?php if ( true === self::are_steps_missing() ) : ?>
								<span><?php _e( 'Run Missing Steps', 'wpcasa-oslo' ); ?></span>
							<?php else: ?>
								<span><?php _e( 'Run Demo Import', 'wpcasa-oslo' ); ?></span>
							<?php endif; ?>
						</a>
					...
					<?php else : ?>
						...
						<div class="wpsight-demo-import-notice notice notice-error">
							...
							<p><strong><?php _e( 'Plugins Missing!', 'wpcasa-oslo' ); ?></strong>
							<br /><?php _e( 'Please install and activate the plugins listed below. They are necessary to complete the import process.', 'wpcasa-oslo' ); ?></p>
