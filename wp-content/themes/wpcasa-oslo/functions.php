 ...
 */
add_action( 'after_setup_theme', 'wpsight_oslo_constants' );

function wpsight_oslo_constants() {
	...
	define( 'WPSIGHT_OSLO_NAME', 'WPCasa Oslo' );
	define( 'WPSIGHT_OSLO_DOMAIN', 'wpcasa-oslo' );
	define( 'WPSIGHT_OSLO_VERSION', '1.0.1' );
 ...
 */
add_action( 'wp_enqueue_scripts', 'wpsight_oslo_scripts' );

function wpsight_oslo_scripts() {
	
	// Script debugging?
	$suffix = SCRIPT_DEBUG ? '' : '.min';
	
	// Enqueue theme CSS
	wp_enqueue_style( 'wpcasa-oslo', get_stylesheet_uri() );
	
	// Enqueue theme JS
	wp_enqueue_script( 'wpcasa-oslo', get_template_directory_uri() . '/assets/js/wpcasa-oslo.js', array( 'jquery' ), WPSIGHT_OSLO_VERSION, true );
	
	// Enqueue Bootstrap library if desired
	
	if( apply_filters( 'wpsight_oslo_bootstrap', true ) == true ) {
	...
	if( apply_filters( 'wpsight_oslo_pushy', true ) == true ) {		
		wp_enqueue_style( 'pushy', get_template_directory_uri() . '/assets/css/pushy.css', false, WPSIGHT_OSLO_VERSION, 'all'  );
	...
	if( apply_filters( 'wpsight_oslo_lightslider', true ) == true ) {		
	...
	if( apply_filters( 'wpsight_oslo_lightgallery', true ) == true ) {		
	...
	if( apply_filters( 'wpsight_oslo_equal_height', true ) == true ) {
	...
	if( apply_filters( 'wpsight_oslo_smooth_scroll', true ) == true )
	...
	// Enqueue animate.css
	wp_enqueue_style( 'animate', get_template_directory_uri() . '/assets/css/animate' . $suffix . '.css', false, WPSIGHT_OSLO_VERSION, 'all'  );
	...
	if( apply_filters( 'wpsight_oslo_google_fonts', true ) == true ) {
 ...
 */
add_action( 'after_setup_theme', 'wpsight_oslo_theme_updater' );

function wpsight_oslo_theme_updater() {
