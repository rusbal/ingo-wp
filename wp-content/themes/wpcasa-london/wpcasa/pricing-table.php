...
<section class="wpsight-pricing-table">
	...
	<?php if( ! empty( $pricing_plans ) ) : ?>
		...
		<div class="pricing-plans row gutter-40">
		
			<?php
				if( count( $pricing_plans ) == 4 ) {
					$pricing_plan_class = 'col-xs-12 col-sm-6 col-md-3';
				} elseif( count( $pricing_plans ) == 3 ) {
					$pricing_plan_class = 'col-xs-12 col-md-4';
				} elseif( count( $pricing_plans ) == 2 ) {
					$pricing_plan_class = 'col-xs-12 col-md-6';
				} else {
					$pricing_plan_class = 'col-xs-12';
				}
			?>
		
			<?php foreach( (array) $pricing_plans as $pricing_plan ) : ?>
			
				<div id="pricing-plan-wrap-<?php echo sanitize_title( $pricing_plan['pricing_plan_title'] ); ?>" class="pricing-plan-wrap <?php echo esc_attr( $pricing_plan_class ); ?>">
					...
					<div id="pricing-plan-<?php echo sanitize_title( $pricing_plan['pricing_plan_title'] ); ?>" class="pricing-plan">
						...
						<?php if( ! empty( $pricing_plan['pricing_plan_price'] ) || 0 == $pricing_plan['pricing_plan_price'] ) : ?>
						...
						<div class="pricing-plan-action">
							...
							<?php else : ?>
								...
								<?php if( ! empty( $pricing_plan['pricing_plan_button_url'] ) ) : ?>
									<a href="<?php echo esc_url( $pricing_plan['pricing_plan_button_url'] ); ?>" class="btn btn-primary btn-block" role="button"><?php echo strip_tags( $button_text ); ?></a>
								<?php endif; ?>
						...
						<div class="corner-ribbon top-right shadow"><?php echo strip_tags( $pricing_plan['pricing_plan_ribbon'] ); ?></div>
						<?php endif; ?>
