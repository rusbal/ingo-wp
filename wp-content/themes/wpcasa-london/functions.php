 ...
 */
add_action( 'after_setup_theme', 'wpsight_london_constants' );

function wpsight_london_constants() {
	...
	define( 'WPSIGHT_LONDON_NAME', 'WPCasa London' );
	define( 'WPSIGHT_LONDON_DOMAIN', 'wpcasa-london' );
	define( 'WPSIGHT_LONDON_VERSION', '1.0.2' );
 ...
 */
add_action( 'wp_enqueue_scripts', 'wpsight_london_scripts' );

function wpsight_london_scripts() {
	
	// Script debugging?
	$suffix = SCRIPT_DEBUG ? '' : '.min';
	
	// Enqueue theme CSS
	wp_enqueue_style( 'wpcasa-london', get_stylesheet_uri() );
	
	// Enqueue theme JS
	wp_enqueue_script( 'wpcasa-london', get_template_directory_uri() . '/assets/js/wpcasa-london.js', array( 'jquery' ), WPSIGHT_LONDON_VERSION, true );
	
	// Enqueue Bootstrap library if desired
	
	if( apply_filters( 'wpsight_london_bootstrap', true ) == true ) {
	...
	if( apply_filters( 'wpsight_london_pushy', true ) == true ) {		
		wp_enqueue_style( 'pushy', get_template_directory_uri() . '/assets/css/pushy.css', false, WPSIGHT_LONDON_VERSION, 'all'  );
	...
	if( apply_filters( 'wpsight_london_lightslider', true ) == true ) {		
	...
	if( apply_filters( 'wpsight_london_lightgallery', true ) == true ) {		
	...
	if( apply_filters( 'wpsight_london_equal_height', true ) == true ) {
		$other_suffix = SCRIPT_DEBUG ? '' : '-min';
		wp_enqueue_script( 'jquery-match-height', get_template_directory_uri() . '/vendor/liabru/jquery-match-height/jquery.matchHeight' . $other_suffix . '.js', array( 'jquery' ), '0.7.0', false );
	}
	
	// Enqueue Smooth Scroll if desired
	
	if( apply_filters( 'wpsight_london_smooth_scroll', true ) == true )
		wp_enqueue_script( 'jquery-smooth-scroll', get_template_directory_uri() . '/assets/js/jquery.smooth-scroll.min.js', array( 'jquery' ), '1.5.6', false );
	
	// Enqueue animate.css
	wp_enqueue_style( 'animate', get_template_directory_uri() . '/assets/css/animate' . $suffix . '.css', false, WPSIGHT_LONDON_VERSION, 'all'  );
	...
	if( apply_filters( 'wpsight_london_google_fonts', true ) == true ) {
 ...
 */
add_action( 'after_setup_theme', 'wpsight_london_theme_updater' );

function wpsight_london_theme_updater() {
