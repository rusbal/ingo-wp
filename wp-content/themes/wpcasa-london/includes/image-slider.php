...
/**
 * wpsight_london_image_slider()
 *
 * Echo wpsight_london_get_slider()
 *
 * @param integer|array $post_id Post ID or array of attachment IDs
 * @param array $args Optional slider arguments
 *
 * @uses wpsight_london_get_image_slider()
 *
 * @since 1.0.0
 */
function wpsight_london_image_slider( $post_id = '', $args = array() ) {
	echo wpsight_london_get_image_slider( $post_id, $args );
}

/**
 * wpsight_london_get_image_slider()
 *
 * Create image slider.
 *
 * @param integer|array $post_id Post ID or array of attachment IDs
 ...
 * @uses wpsight_london_image_slider_js()
 *
 * @return string HTML markup of image slider
 *
 * @since 1.0.0
 */
function wpsight_london_get_image_slider( $post_id = '', $args = array() ) {
	...
	if( ! is_array( $post_id ) ) {
		...
		// Set slider ID
		$slider_id = 'wpsight-slider-' . absint( $post_id );
	...
	// Else set array of image attachment IDs
		...
		// Set slider ID
		$slider_id = 'wpsight-slider-' . implode( '-', $images );
	...
	if( $images ) {
		...
		$default_class_slider	= isset( $args['class_slider'] ) ? $args['class_slider'] : $slider_id . ' wpsight-image-slider';
		...
		$args = apply_filters( 'wpsight_london_get_image_slider_args', wp_parse_args( $args, $defaults ), $post_id, $args );
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
    			...
    			$('#<?php echo esc_attr( $slider_id ); ?>-wrap .wpsight-image-slider-prev-next').insertAfter('#<?php echo esc_attr( $slider_id ); ?>');
    			<?php endif; ?>
			...
			<ul id="<?php echo esc_attr( $slider_id ); ?>" class="<?php echo $class_slider; ?>" itemscope itemtype="http://schema.org/ImageGallery">
				...
				<?php foreach( $images as $image ) : ?>
					...
					<?php if( $attachment !== NULL && $attachment->post_type == 'attachment' ) : ?>
						...
						<li data-thumb="<?php echo esc_url( $attachment_thumb[0] ); ?>">
						
							<figure class="<?php echo $class_item; ?>" itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">
						
								<img src="<?php echo esc_url( $attachment_src[0] ); ?>" itemprop="thumbnail" alt="<?php echo esc_attr( get_post_meta( $attachment->ID, '_wp_attachment_image_alt', true ) ); ?>" />
								...
								<?php if( $attachment->post_excerpt && $args['image_caption'] === true ) : ?>
		...
		return apply_filters( 'wpsight_london_get_image_slider', ob_get_clean(), $post_id, $args, $slider_id );
