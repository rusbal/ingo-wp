...
add_action( 'widgets_init', 'wpsight_london_register_widget_listings_carousel' );
 
function wpsight_london_register_widget_listings_carousel() {
	register_widget( 'WPSight_London_Listings_Carousel' );
}
...
class WPSight_London_Listings_Carousel extends WP_Widget {

	public function __construct() {

		$widget_ops = array(
			'classname'   => 'widget_listings_carousel',
			'description' => _x( 'Display listings in a carousel.', 'listing wigdet', 'wpcasa-london' )
		);

		parent::__construct( 'wpsight_london_Listings_Carousel', '&rsaquo; ' . _x( 'Listings Carousel', 'listing widget', 'wpcasa-london' ), $widget_ops );
	...
	public function widget( $args, $instance ) {
		...
		$listings_args = apply_filters( 'wpsight_london_widget_listings_carousel_query_args', array_merge( $listings_args, $taxonomy_filters ), $instance, $this->id );
		...
		wpsight_london_listings_carousel( $listings, $carousel_args );
	...
	public function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'carousel_items' ); ?>"><?php _e( 'Items', 'wpcasa-london' ); ?>:</label><br />
			<select class="widefat" id="<?php echo $this->get_field_id( 'carousel_items' ); ?>" name="<?php echo $this->get_field_name( 'carousel_items' ); ?>">
				<option value="1"<?php selected( $carousel_items, 1 ); ?>>1 <?php _ex( 'item', 'listing widget', 'wpcasa-london' ); ?></option>
				<option value="2"<?php selected( $carousel_items, 2 ); ?>>2 <?php _ex( 'items', 'listing widget', 'wpcasa-london' ); ?></option>
				<option value="3"<?php selected( $carousel_items, 3 ); ?>>3 <?php _ex( 'items', 'listing widget', 'wpcasa-london' ); ?></option>
				<option value="4"<?php selected( $carousel_items, 4 ); ?>>4 <?php _ex( 'items', 'listing widget', 'wpcasa-london' ); ?></option>
				<option value="5"<?php selected( $carousel_items, 5 ); ?>>5 <?php _ex( 'items', 'listing widget', 'wpcasa-london' ); ?></option>
				<option value="6"<?php selected( $carousel_items, 6 ); ?>>6 <?php _ex( 'items', 'listing widget', 'wpcasa-london' ); ?></option>
			</select><br />
			<span class="description"><?php _e( 'Please select the number of visible carousel items', 'wpcasa-london' ); ?></span></p>
		
		<div<?php if( $carousel_items == 1 ) echo ' style="display:none"'; ?>>
			
			<p><label for="<?php echo $this->get_field_id( 'carousel_slide_by' ); ?>"><?php _e( 'Slide by', 'wpcasa-london' ); ?>:</label><br />
				<select class="widefat" id="<?php echo $this->get_field_id( 'carousel_slide_by' ); ?>" name="<?php echo $this->get_field_name( 'carousel_slide_by' ); ?>">
					<option value="1"<?php selected( $carousel_slide_by, 1 ); ?>>1 <?php _ex( 'item', 'listing widget', 'wpcasa-london' ); ?></option>
					<option value="2"<?php selected( $carousel_slide_by, 2 ); if( $carousel_items < 2 ) echo ' style="display:none"'; ?>>2 <?php _ex( 'items', 'listing widget', 'wpcasa-london' ); ?></option>
					<option value="3"<?php selected( $carousel_slide_by, 3 ); if( $carousel_items < 3 ) echo ' style="display:none"'; ?>>3 <?php _ex( 'items', 'listing widget', 'wpcasa-london' ); ?></option>
					<option value="4"<?php selected( $carousel_slide_by, 4 ); if( $carousel_items < 4 ) echo ' style="display:none"'; ?>>4 <?php _ex( 'items', 'listing widget', 'wpcasa-london' ); ?></option>
					<option value="5"<?php selected( $carousel_slide_by, 5 ); if( $carousel_items < 5 ) echo ' style="display:none"'; ?>>5 <?php _ex( 'items', 'listing widget', 'wpcasa-london' ); ?></option>
					<option value="6"<?php selected( $carousel_slide_by, 6 ); if( $carousel_items < 6 ) echo ' style="display:none"'; ?>>6 <?php _ex( 'items', 'listing widget', 'wpcasa-london' ); ?></option>
				</select><br />
				<span class="description"><?php _e( 'Please select the number items to slide', 'wpcasa-london' ); ?></span></p>
		
		</div>
		
		<p><label for="<?php echo $this->get_field_id( 'carousel_margin' ); ?>"><?php _e( 'Margin', 'wpcasa-london' ); ?>:</label><br />
			...
			<span class="description"><?php _e( 'Please select the space between carousel items', 'wpcasa-london' ); ?></span></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'carousel_loop' ); ?>" name="<?php echo $this->get_field_name( 'carousel_loop' ); ?>"<?php checked( $carousel_loop ); ?> />
				<label for="<?php echo $this->get_field_id( 'carousel_loop' ); ?>"><?php _e( 'Loop carousel to be infinite', 'wpcasa-london' ); ?></label></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'carousel_nav' ); ?>" name="<?php echo $this->get_field_name( 'carousel_nav' ); ?>"<?php checked( $carousel_nav ); ?> />
				<label for="<?php echo $this->get_field_id( 'carousel_nav' ); ?>"><?php _e( 'Show prev/next carousel navigation', 'wpcasa-london' ); ?></label></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'carousel_dots' ); ?>" name="<?php echo $this->get_field_name( 'carousel_dots' ); ?>"<?php checked( $carousel_dots ); ?> />
				<label for="<?php echo $this->get_field_id( 'carousel_dots' ); ?>"><?php _e( 'Show <em>dots</em> carousel navigation', 'wpcasa-london' ); ?></label></p>
		...
		<p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'carousel_autoplay' ); ?>" name="<?php echo $this->get_field_name( 'carousel_autoplay' ); ?>"<?php checked( $carousel_autoplay ); ?> />
				<label for="<?php echo $this->get_field_id( 'carousel_autoplay' ); ?>"><?php _e( 'Slide through items automatically', 'wpcasa-london' ); ?></label></p>
		
		<div<?php if( ! $carousel_autoplay ) echo ' style="display:none"'; ?>>
		
			<p><label for="<?php echo $this->get_field_id( 'carousel_autoplay_time' ); ?>"><?php _e( 'Autoplay interval', 'wpcasa-london' ); ?>:</label><br />
				<select class="widefat" id="<?php echo $this->get_field_id( 'carousel_autoplay_time' ); ?>" name="<?php echo $this->get_field_name( 'carousel_autoplay_time' ); ?>">
					<option value="2000"<?php selected( $carousel_autoplay_time, 2000 ); ?>>2 <?php _ex( 'seconds', 'listing widget', 'wpcasa-london' ); ?></option>
					<option value="4000"<?php selected( $carousel_autoplay_time, 4000 ); ?>>4 <?php _ex( 'seconds', 'listing widget', 'wpcasa-london' ); ?></option>
					<option value="6000"<?php selected( $carousel_autoplay_time, 6000 ); ?>>6 <?php _ex( 'seconds', 'listing widget', 'wpcasa-london' ); ?></option>
					<option value="8000"<?php selected( $carousel_autoplay_time, 8000 ); ?>>8 <?php _ex( 'seconds', 'listing widget', 'wpcasa-london' ); ?></option>
					<option value="10000"<?php selected( $carousel_autoplay_time, 10000 ); ?>>10 <?php _ex( 'seconds', 'listing widget', 'wpcasa-london' ); ?></option>
				</select><br />
				<span class="description"><?php _e( 'Please select the autoplay interval timeout', 'wpcasa-london' ); ?></span></p>
