 ...
 * Register widget
 */
add_action( 'widgets_init', 'wpsight_london_register_widget_newsletter_box' );
 
function wpsight_london_register_widget_newsletter_box() {
	register_widget( 'WPSight_London_Newsletter_Box' );
}

/**
 * Newsletter box widget class
 */
class WPSight_London_Newsletter_Box extends WP_Widget {

	public function __construct() {

		$widget_ops = array(
			'classname'   => 'widget_newsletter_box',
			'description' => _x( 'Display a newsletter box.', 'listing wigdet', 'wpcasa-london' )
		);

		parent::__construct( 'wpsight_london_newsletter_box', '&rsaquo; ' . _x( 'Newsletter Box', 'listing widget', 'wpcasa-london' ), $widget_ops );
	...
	public function widget( $args, $instance ) {
		...
		$defaults = array(
			'title'			=> __( 'Signup for Our Newsletter', 'wpcasa-london' ),
			'description'	=> __( 'Stay updated and get our latest news right into your inbox. No spam.', 'wpcasa-london' ),
			'form'			=> '',
		...
		// Display feature box
		wpsight_london_newsletter_box( $instance, $args );
	...
	public function form( $instance ) {
		...
		<textarea class="widefat" rows="6" cols="20" id="<?php echo $this->get_field_id( 'form' ); ?>" name="<?php echo $this->get_field_name( 'form' ); ?>"><?php echo esc_textarea( $instance['form'] ); ?></textarea></label><br />
			<span class="description"><?php _e( 'Place the newsletter form code or shortcode here. We recommend to use <a href="https://wordpress.org/plugins/mailchimp-for-wp/" target="_blank">Mailchimp for WordPress</a>', 'wpcasa-london' ); ?></span></p>
		...
		<p><label for="<?php echo $this->get_field_id( 'icon_class' ); ?>"><?php _e( 'Icon Class', 'wpcasa-london' ); ?>: <input class="widefat" id="<?php echo $this->get_field_id( 'icon_class' ); ?>" name="<?php echo $this->get_field_name( 'icon_class' ); ?>" type="text" value="<?php echo esc_attr( $instance['icon_class'] ); ?>" /></label><br />
			<span class="description"><?php _e( 'For example <a href="https://fortawesome.github.io/Font-Awesome/icons/" target="_blank">Font Awesome</a>', 'wpcasa-london' ); ?></span></p><?php
